...
const page = usePage();
const notifications = (page.props.notifications as Notification[] | undefined) ?? [];
        ...
        <DropdownMenu>
            <DropdownMenuTrigger as-child>
                <SidebarMenuButton size="lg" class="relative w-full">
                    ...
                    <span v-if="notifications.length" class="inline-flex h-2 w-2 rounded-full bg-red-500"></span>
                </SidebarMenuButton>
            </DropdownMenuTrigger>
            <DropdownMenuContent class="w-80">
                ...
                <template v-if="notifications.length">
                    <DropdownMenuItem v-for="notification in notifications" :key="notification.id" as-child>
                        <div class="flex flex-col items-start">
                            <span class="text-sm">{{ notification.data.message }}</span>
                            <span class="text-xs text-muted-foreground">
                                {{ formatDate(notification.created_at) }}
                            </span>
                            <button v-if="notification.data.ticket_id" @click="markAsReadAndGo(notification)"
                                class="text-primary underline text-xs mt-1">
                                View
                            </button>
                        </div>
                    </DropdownMenuItem>
