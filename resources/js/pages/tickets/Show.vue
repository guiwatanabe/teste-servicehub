        ...
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
            <div class="flex justify-between items-center">
                ...
                <div v-if="ticket.status !== 'closed'">
                    <button class="rounded-md border border-red-400 dark:border-red-700 px-3 py-1 hover:bg-muted"
                        @click="closeTicket">
                        Close Ticket
                    </button>
                </div>
                ...
                <div class="p-4">
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div>
                                <h3 class="text-sm font-semibold text-muted-foreground">Project</h3>
                                <p class="text-sm">{{ ticket.project?.title ?? '-' }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-semibold text-muted-foreground">Assigned To</h3>
                                <p class="text-sm">{{ ticket.recipient?.name ?? '-' }}</p>
                            </div>
                            <div>
                                <h3 class="text-sm font-semibold text-muted-foreground">Created</h3>
                                <p class="text-sm">{{ ticket.created_at }}</p>
                            </div>
                            <div v-if="ticket.status == 'closed'">
                                <h3 class="text-sm font-semibold text-muted-foreground">Closed At</h3>
                                <p :class="getDateColor(ticket.status, ticket.due_date ?? null)" class="text-sm">{{
                                    ticket.closed_at }}</p>
                            </div>
                            <div v-else>
                        ...
                        <div>
                            ...
                            <div>
                                <TicketDetailsInfo v-if="ticket.ticket_detail" :ticketDetail="ticket.ticket_detail" />
