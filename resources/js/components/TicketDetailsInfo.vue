...
<template>
	<div v-if="ticketDetail.details" class="border border-border p-2">
		...
		<div v-else>
			<ul v-if="parsedContent && typeof parsedContent === 'object'" class="space-y-2 text-sm">
				<li v-for="(value, key) in parsedContent" :key="key"
					class="rounded-md border border-border/60 bg-muted/30 p-2">
					<div class="flex flex-wrap items-center gap-2">
						<span
							class="w-full rounded bg-muted px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
							{{ key }}
						</span>
						<span v-if="Array.isArray(value)" class="text-foreground">
							<ul class="mt-2 space-y-1">
								<li v-for="(item, index) in value" :key="index" class="text-foreground">
									{{ renderValue(item) }}
								</li>
							</ul>
						</span>
						<span v-else-if="typeof value === 'object'" class="w-full">
							<pre
								class="mt-1 whitespace-pre-wrap rounded bg-background/60 p-2 text-xs text-foreground">{{ JSON.stringify(value, null, 2) }}</pre>
						</span>
