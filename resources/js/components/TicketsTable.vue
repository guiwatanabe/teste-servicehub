    ...
    <table class="w-full text-left text-sm">
        ...
        <tbody>
            ...
            >
                ...
                <td v-if="showProject" class="max-w-[140px] py-3 pr-4">
                    <span class="block truncate">
                ...
                <td class="py-3 pr-4 text-nowrap">
                    <span
                        :class="`rounded p-1 font-bold ${getPriorityColor(ticket.priority)}`"
                        >{{ ticket.priority }}</span
                    >
                </td>
                <td class="py-3 pr-4 text-nowrap">
                    <span
                        :class="`rounded p-1 font-bold ${getStatusColor(ticket.status)}`"
                        >{{ ticket.status }}</span
                    >
                </td>
                <td class="py-3 text-nowrap">
                    <span
                        :class="
                            getDateColor(ticket.status, ticket.due_date ?? null)
                        "
